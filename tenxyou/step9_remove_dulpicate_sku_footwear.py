...
def remove_duplicate_skus(connection_string, database_name, collection_name, date):
    ...
    print(f"Removed {sum(len(item['ids']) - 1 for item in duplicates)} duplicate SKUs for on {date}.")

def start_step11():
    # Example usage
    connection_string = "mongodb://localhost:27017"
    database_name = 'tg_analytics_tenxyou'

    # Get today's date and format it
    today_str = datetime.today().strftime("%Y-%m-%d") 
    # today_str = '2025-12-09'

    collections = [f'crawler_sink_tenxyou_india_footwear']

    dates = os.listdir('India/Data')
