...
def check_data_format(connection_string, database_name, collection_name, geography, date):
    ...
    if any(age_group not in ['new_born', 'baby', 'junior', 'senior', 'teen', 'adult', 'Adult', 'kids'] for age_group in age_groups):
        format_check = 1
        logging.error(f"Data format error in {geography} for date {date}: 'age_group' contains invalid values.{age_groups}")
    if any(age_range not in ['0m', '1m', '2m', '3m', '4m', '5m', '6m', '7m', '8m', '9m', '10m', '11m', '12m', '13m', '14m', '15m', '16m', '17m', '18m', '19m', '20m', '21m', '22m', '23m', '24m', '1y', '2y', '3y', '4y', '5y', '6y', '7y', '8y', '9y', '10y', '11y', '12y', '13y', '14y', '15y', '16y', '17y', '18y'] for age_range in age_ranges):
        format_check = 1
        logging.error(f"Data format error in {geography} for date {date}: 'age_range' contains invalid values.{age_ranges}")
...
if __name__ == "__main__":
    ...
    if day in ['Monday', 'Wednesday', 'Friday']:
        collections = {
            'UK' : ['crawler_sink_skechers_uk_footwear', 'crawler_sink_skechers_uk'],
            'USA' : ['crawler_sink_skechers_usa_footwear', 'crawler_sink_skechers_usa']
        }
    elif day in ['Tuesday', 'Thursday', 'Saturday']:
        collections = {
            'India' : ['crawler_sink_skechers_india_footwear', 'crawler_sink_skechers_india']
        }
    else:
        collections = {}
