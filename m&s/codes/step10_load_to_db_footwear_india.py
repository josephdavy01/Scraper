...
def process_data(json_data, today_date, gender):
    ...
    more_details = details_and_cares.get('more_details', [])
    
    # Check generic type
    generic_type = None
    for item in more_details:
        if item and item.startswith("Generic Type:"):
            generic_type = item.split("Generic Type:")[1].strip().lower()
            break
    if generic_type != "footwear":
        return []  # Skip non-footwear

    # Toe type
    toe_type = None
    for item in more_details:
        if item and item.startswith("Toe Shape:"):
            toe_type = item.split("Toe Shape:")[1].strip()
            break

    # Heel to toe drop
    heel_to_toe_drop = None
    size_and_fit = details_and_cares.get("Size & Fit", [])
    for item in size_and_fit:
        if "Heel Height" in item:
            match = re.search(r'(\d+mm)', item)
            if match:
                heel_to_toe_drop = match.group(1)
                break

    # Sizes
    sizes = data.get('product_size', {})
    valid_sizes = {k: v for k, v in sizes.items() if not (k == 'Select Size' or k.startswith('QTY'))}
    if not valid_sizes:
        return []
    ...
    # Price
    prizes = details_and_cares.get('prize', {'current_prize': 0, 'old_prize': 0})
    ...
    name_of_origin = None
    for item in more_details:
        if item and item.startswith('Country of origin:'):
            country_text = item.split('Country of origin:')[1].strip()
            first_country = country_text.split('/')[0].strip().lower()
            name_of_origin = first_country

    date_of_scraping = parse_launch_date(today_date)

    # Generate entries per size
    for size_key, size_url in valid_sizes.items():  # iterate key, value
    # Determine availability
        if 'Out of Stock' in size_key:
            availability = 'out_of_stock'
            size_name_clean = re.sub(r'\s*Out of Stock\s*', '', size_key).strip()
        elif 'Low in Stock' in size_key:
            availability = 'low_in_stock'
            size_name_clean = re.sub(r'\s*Low in Stock\s*', '', size_key).strip()
        else:
            availability = 'in_stock'
            size_name_clean = size_key.strip()

        size_name_clean = size_name_clean.replace(' ', '')

        entry = {
            "product_id": product,
            "sub_brand": data.get("Brand_name", "").strip().lower(),
            "gender": gender.lower(),
            "age_group": ["adult"],
            "age_range": ["18y"],
            ...
            "sku": f'{product}%{product_id}c{color_id}s{size_name_clean}',
            "size_name": size_name_clean,
            "size_ref_code": None,
            "price": current_prize,
