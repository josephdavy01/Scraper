...
def process_data(json_data, today_date, gender):
    ...
    brand_name = data.get("Brand_name", "").strip().lower() if isinstance(data.get("Brand_name"), str) else ""
    ...
    reference_code = product_description_details.get('product_id', '')
    include_base = [
        "slipper", "shoe", "boot", "sandle", "trainer", 
        "pump", "loafer", "flat", "slider", "mule"
    ]
    include_keywords = [w for word in include_base for w in (word, word + "s")]
    pattern = re.compile(r"\b(" + "|".join(include_keywords) + r")\b", re.IGNORECASE)
    if not pattern.search(page_title.lower()):
        print(f"Skipping product due to missing keyword in title: {product_id} ({page_title})")
        return []
    toe_type = None
    details_list = data.get("details_and_cares", {}).get("details", [])
    for d in details_list:
        if "toe style" in d.lower():
            toe_type = d.split(":")[-1].strip().lower() 
    heel_to_toe_drop = None
    size_fit_list = data.get("details_and_cares", {}).get("size_and_fit", [])
    for s in size_fit_list:
        if "heel height" in s.lower():
            match = re.search(r'(\d+\s*mm)', s.lower())
            if match:
                heel_to_toe_drop = match.group(1)  # Keep as string, e.g., "19mm"
            else:
                heel_to_toe_drop = s.split(":")[-1].strip()
    upper_material, sole_material = None, None
    details_and_cares = data.get("details_and_cares", {})
    for c in details_and_cares.get("care", []):
        if c.lower().startswith("upper:"):
            upper_material = c.split(":", 1)[-1].strip()
        elif c.lower().startswith("outsole:"):
            sole_material = c.split(":", 1)[-1].strip()
    ...
    for size_key, size_val in valid_sizes.items():
        ...
        if 'Out of Stock' in size_key:
            ...
            size_key_clean = re.add(r'\s*Out of Stock\s*', '', size_key).strip()
        ...
        # Extract US size (e.g., "US 5 (UK 7)" → "US5")
        us_match = re.search(r'US\s*(\d+)', size_key_clean, flags=re.IGNORECASE)
        if us_match:
            size_name = f"US{us_match.group(1)}"
        else:
            size_name = size_key_clean
            print(f"Non-US size detected for {product_id}: {size_name}")
        ...
        temp.append(entry)
    print(f"Processed entries for product {product_id}: {len(temp)} entries")
    return temp
...
def process_jsons(base_dir, scraping_date, collection):
    ...
    if not genders:
        print(f"No gender folders found in {base_dir}")
    for gender in genders:
        ...
        if not os.path.isdir(gender_folder):
            print(f"Skipping {gender_folder}: Not a directory")
            continue
        print(f"Processing gender folder: {gender_folder}")
        categories = os.listdir(gender_folder)
        if not categories:
            print(f"No category folders found in {gender_folder}")
        for category in categories:
            category_folder = os.path.join(gender_folder, category)
            if not os.path.isdir(category_folder):
                print(f"Skipping {category_folder}: Not a directory")
                continue
            print(f"Processing category folder: {category_folder}")
            files = [f for f in os.listdir(category_folder) if f.endswith('.json')]
            if not files:
                print(f"No JSON files found in {category_folder}")
            for file in files:
                ...
                try:
                    ...
                    if temp:
                        try:
                            collection.insert_many(temp, ordered=False)
                            print(f"Inserted {len(temp)} documents for {file_path}")
                        ...
                        print(f"No valid data to insert for {file_path}")
        ...
        for date_folder in folders_to_process:
            try:
                ...
                if not single_day and folder_date >= today_date:
                    print(f"Skipping future folder: {date_folder}")
                    continue
            except ValueError:
