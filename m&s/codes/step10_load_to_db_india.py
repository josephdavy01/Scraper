...
def process_data(json_data, today_date, gender):
    ...
    skip_base = [
        ...
        "slipper", "slider", "shoe", "heal", "pump", "sock", "handkerchief", "tie", "brim",
    ...
    new_origin = details_and_cares.get('more_details', [])
    new_origin = details_and_cares.get('more_details', [])
    name_of_origin = None
    if new_origin:
        for item in new_origin:
    ...
    for size_key, size_value in valid_sizes.items():
        if 'Out of Stock' in size_key:
            availability = 'out_of_stock'
            size_name = re.sub(r'\s*Out of Stock\s*', '', size_key).strip()
        elif 'Low in Stock' in size_key:
            availability = 'low_in_stock'
            size_name = re.sub(r'\s*Low in Stock\s*', '', size_key).strip()
        else:
            availability = 'in_stock'
            size_name = size_key.strip()
        entry = {
            "product_id": product,
