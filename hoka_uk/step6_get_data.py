...
def setup_context(pw, proxy_str=None):
    ...
    if proxy_str:
        # proxy_str example: "http://username:password@host:port" or "http://host:port"
...
def get_product_data(page, url):
    ...
    color_name = color_name_tag.text.strip() if color_name_tag else "Unknown"
    json_data['color_id'] = color_id
    json_data['color_name'] = color_name
    ...
    for row in soup.find_all("div", class_="row pb-4 align-items-center"):
        ...
        if label and "Weight" in label.get_text(strip=True):
    ...
    for row in soup.find_all("div", class_="row pb-4 align-items-center"):
        label = row.find("div", class_="col-3 col-md-4")
        value = row.find("div", class_="col detail-col")
        if label and "Heel-To-Toe Drop" in label.get_text(strip=True):
...
def process_products(input_path, country, today_str):
    all_product_urls = load_product_urls(input_path)

    # Pre-filter: Only keep URLs that need scraping
    pending_urls = []
    for main_category, sub_category, url in all_product_urls:
        path_name = url.split('?')[0].split('/')[-1].split('.')[0]
        query_params = parse_qs(urlparse(url).query)
        color = query_params.get(f'dwvar_{path_name}_color', ["default"])[0]
        name = f"{path_name}_{color}"
        gender = main_category
        category = sub_category if sub_category else main_category
        json_path = Path(country) / "Data" / today_str / "Json_Data" / gender / category / f"{name}.json"

        if not json_path.exists():
            pending_urls.append((main_category, sub_category, url))
        else:
            logging.info(f"Skipping {url} - JSON already exists")

    logging.info(f"{len(pending_urls)} products remaining to scrape out of {len(all_product_urls)}")

    batch_size = 2

    with sync_playwright() as pw:
        i = 0
        while i < len(pending_urls):
            # Start fresh browser session
            context, browser, page = setup_context(pw)

            if not safe_visit(page, MAIN_PAGE, pw, []):
                logging.error("Failed to open HOKA main page with cookies.")
                context.close()
                browser.close()
                time.sleep(5)
            ...
            for main_category, sub_category, url in batch:
                try:
                    logging.info(f"Scraping ({sub_category}) - {url}")
                    time.sleep(random.uniform(5, 10))

                    name, data = get_product_data(page, url)
                    save_json(main_category, sub_category, name, data, country, today_str)

                    # After each scrape, revisit main page to refresh session
                    safe_visit(page, MAIN_PAGE, pw, [])
...
# ─────────────────────────────────────────────
# Main entry
def main(country: str, today_str: str):
