...
def is_footware(json_data):
    ...
    for size in availibility.keys():
        ...
        if size_clean in clothing_sizes:
            return False
    return True
...
def create_individual_json(today_str, json_data, gender):
    ...
    if key_features:
        descriptions += '\n' + ' | '.join(key_features)
    heel_drop = json_data.get('heel_to_toe_drop', '')
    prices = extract_prices(product)
    price = prices["price"]
    launch_price = prices["launch_price"]
    weight = json_data.get('weight', '')
    occasion = json_data.get('occasion', '').lower()
    if occasion == '':
        occasion = None
    ...
    for size, status in availibility.items():
        ...
        size_availability = status.lower()
        size_specific_sku = f"{product_id}%p{product_id.replace('hok', '')}c{cid}s{size_name}"
        if size_availability == "in stock":
            availability = "in_stock"
        elif size_availability == "low stock":
...
if __name__ == "__main__":
    ...
    for country in countries:
        collection = db[f'crawler_sink_hoka_{country.lower()}_footwear']
