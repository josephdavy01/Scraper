...
BASE_DIR = f"UK/data/{today}/Json_data"
URL_FILE = f"UK/data/{today}/Item_urls/unique_product_urls.json"
VARIANT_FILE = f"UK/data/{today}/Item_urls/variant_urls.json"
os.makedirs(BASE_DIR, exist_ok=True)
os.makedirs(os.path.dirname(VARIANT_FILE), exist_ok=True)


# -------------------------------------------------------
# NEW FUNCTION — Skip already scraped URLs
# -------------------------------------------------------
def is_already_scraped(main_cat, sub_cat, url):
    parsed = urlparse(url)
    ...
    file_path = os.path.join(BASE_DIR, main_cat, sub_cat, f"{product_id}_{color_id}.json")
...
def extract_price(soup):
    ...
    list_price = price_div.get("data-list-price")
    sale_price = price_div.get("data-sales-price")

    if list_price:
        try:
            price_data["original_price"] = float(list_price)
        except ValueError:
            pass

    if sale_price:
        try:
            price_data["sale_price"] = float(sale_price)
        except ValueError:
            pass

    if not price_data["original_price"]:
        orig_span = price_div.find("span", class_="pricing__base")
        if orig_span:
            text = orig_span.get_text(strip=True).replace("£", "").replace("Original price", "").strip()
            try:
                price_data["original_price"] = float(text)
            except ValueError:
                pass
    ...
    if not price_data["sale_price"]:
        ...
        if sale_span:
            text = sale_span.get_text(strip=True).replace("£", "").strip()
            try:
                price_data["sale_price"] = float(text)
            except ValueError:
                pass
...
def extract_sizes(soup):
    ...
    for size_type in size_types:
        ...
        for li in ul_tag.find_all("li", class_="m-buy-box-grid__item js-product-change-item"):
            ...
            in_stock = "m-buy-box-grid__btn--sold-out" not in btn.get("class", [])
...
def extract_variant_params(soup):
    ...
    ul = options_div.find("ul", class_="m-buy-box-grid m-buy-box-grid-variation-colors js-color-attributes")
    if not ul:
        return variants
    ...
    for li in ul.find_all("li", class_="m-buy-box-grid__item js-product-change-item"):
        ...
        if color_key in q:
            variants.append(f"{color_key}={q[color_key][0]}")
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for idx, (url, main_cat, sub_cat) in enumerate(product_entries, start=1):
            ...
            # ------------------------------------------------
            # NEW: Skip URLs already scraped
            # ------------------------------------------------
            if is_already_scraped(main_cat, sub_cat, url):
                print(f"[{idx}] Already scraped. Skipping: {url}")
                continue
            ...
            if soup:
                ...
                for param_str in variant_params:
                    ...
                    # ------------------------------------------------
                    # NEW: Skip already scraped variant
                    # ------------------------------------------------
                    if is_already_scraped(main_cat, sub_cat, variant_url):
                        print(f"Already scraped variant. Skipping: {variant_url}")
                        continue
