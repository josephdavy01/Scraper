...
CATEGORY_FILE = f"USA/data/{today}/Item_urls/category_urls.json"
PRODUCT_FILE = f"USA/data/{today}/Item_urls/product_urls.json"
...
def load_category_urls():
    try:
        ...
        print(f"Error decoding {CATEGORY_FILE}.")
        return {}
...
async def handle_popups(page: Page):
    try:
        ...
        pass
    try:
        country_popup_close = page.locator(
            "button.country-close, div.country-modal button.close, button#country-modal-close"
        )
        if await country_popup_close.is_visible(timeout=5000):
            print("Country selection popup detected, closing...")
            await country_popup_close.click()
            await random_delay(1, 2)
    except Exception:
        pass
    try:
        subscription_popup_close = page.locator(
            "button.country-close, div.modal-close, button#modal-close, button[title='Close'], a:has-text('No thanks; just browsing')"
        )
        if await subscription_popup_close.is_visible(timeout=5000):
            print("Subscription or offer popup detected, closing...")
            await subscription_popup_close.click()
            await random_delay(1, 2)
    except Exception:
        pass

async def scroll_and_load_products(page: Page, step=800, delay_ms=1000, stable_threshold=5):
    ...
    while scroll_count < max_initial_scrolls:
        ...
        if await load_all_btn.is_visible(timeout=5000):
    ...
    while scroll_count < max_final_scrolls:
        ...
        await asyncio.sleep(delay_ms / 1000)
        await handle_popups(page)
        await page.evaluate(f"window.scrollBy(0, -{step//2});")
    ...
    async with async_playwright() as p:
        ...
        for main_cat, subcats in category_data.items():
            ...
            for cat_name, cat_url in subcats.items():
                ...
                product_items = soup.find_all("li", class_="o-products-grid__item o-products-grid__item--hover-shadow o-products-grid__item--col-1 o-product-grid_hover-index")
                print(f"Found {len(product_items)} product items...")
                for item in product_items:
                    try:
                        anchor = item.find("a", class_="js-grid-anchor")
...
if __name__ == "__main__":
    ...
    if not category_data:
        print("No category data to process. Exiting.")
    else:
