...
def extract_materials(accordion_sections):
    ...
    specs_section = accordion_sections.get('Specs', {})
    details_section = accordion_sections.get('Details', {})
    
    tech_content = tech_section.get('content', '')
    specs_data = specs_section.get('structured_data', {})
    details_data = details_section.get('content', {})


    # Extract weight and heel_drop from the 'Specs' data
    weight = specs_data.get('Weight')
    heel_drop = specs_data.get('Heel Drop')

    return weight, heel_drop,tech_content,details_data
...
def create_individual_json(today_str, json_data, gender):
    ...
    weight, heel_drop,tech_content,details_data = extract_materials(json_data.get('accordion_sections'))
    ...
    description = json_data.get('raw_content', {}).get('product_description', '')
    ...
    # Correctly call format_images on the raw image data
