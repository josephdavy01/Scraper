    ...
    if 'product' in json_data and 'price' in json_data:
        ...
        for color in product.get('colors', []):
            ...
            for size in color.get('sizes', []):
                ...
                # Dynamic Age Logic
                # If the size name contains 'month' or 'year', get the age range and age group
                # Otherwise, set the age group to 'adult' and age range to '18y'
                # Logic: We inspect the size label for age indicators. If found, we extract the numeric
                # age boundaries to categorize the item into groups like 'baby' or 'junior'. 
                # If no age indicators are present, we assume standard adult sizing.
...
def process_apparel(countries, today_date, re_run=False):
    ...
    pop_key = [
        "Accessories", "Backpacks and bags", "Shoes", "Bags", "Belts", "Caps",
        "Fragrances", "Belts and braces", "Caps and gloves", "Jewellery", "More accessories","Hair accessories",
        "Scarves and foulards", "Scarves, caps and gloves", "Sunglasses","Maternity bag","Perfumes",
        "Ties, bow ties and handkerchiefs", "Wallets", "Wallets and cases","Scarves and caps","Scarves and foulards","Scarves, caps and gloves",
        "Socks and slippers", "Hair accessories", "Maternity bag", "Socks and tights","Socks and slippers","Socks and tights",
        "Hats","Backpacks","Backpacks and bags","Bags","Belts","Belts and braces","Caps","Caps and gloves",
        "Mikado diffusers", "Scented candles", "Scarves and caps", "Perfumes","Underwear and socks",
    ]
    blacklist = [k.lower() for k in pop_key]

    for country in countries:
        logging.info(f"Processing apparel for {country}")
        
        data_dir = os.path.join(country, today_date, 'Data')
        if not os.path.exists(data_dir):
            os.makedirs(data_dir)
            
        output_file = os.path.join(data_dir, f'{country}_data_apparel.json')
        error_log_file = os.path.join(data_dir, f'{country}_data_process_apparel_log.json')
        ...
        if not os.path.exists(json_data_dir):
            ...
            # Write logs immediately if critical failure
                    ...
                    for category in categories:
                        ...
                        for file in files:
                            ...
                            try:
                                ...
                                # Check blacklist at category level
                                families = data.get("product", {}).get("families", [])
                                skip_cat = any(c.get("label", "").lower() in blacklist for c in families)
                                if skip_cat:
                                    continue
