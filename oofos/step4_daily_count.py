...
def process_country_data(country, today_str):
    ...
    p2_total = 0
    try:
        file_path_1 = f'{country}/Data/{today_str}/Item_urls/{country}_unique_product_urls.json'
        if not os.path.exists(file_path_1):
            logging.error(f'Error: {file_path_1} file not found.')
            return

        file_path_2 = f'{country}/Data/{today_str}/Item_urls/unique_product_urls.json'
        if not os.path.exists(file_path_2):
            logging.error(f'Error: {file_path_2} file not found.')
            return

        with open(file_path_1) as json_file:
            purls_dict_1 = json.load(json_file)

        with open(file_path_2) as json_file:
            purls_dict_2 = json.load(json_file)

        for gender in purls_dict_1:
            for category, plist in purls_dict_1[gender].items():
        ...
        for gender in purls_dict_2:
            for category, plist in purls_dict_2[gender].items():
                if len(plist) > 0:  # Filter out categories with zero count
                    p2_total += len(plist)
