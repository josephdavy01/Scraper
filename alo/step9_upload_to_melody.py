...
def read_json_data(country, today_str):
    file_path = os.path.join(country, today_str, 'Data', f'{country}_data.json')
...
def upload_to_data_melody(countries, today_str, config):
    ...
    for country in country_list:
        ...
        data_list = read_json_data(country, today_str)
        today_count = len(data_list)

        if today_count == 0:
            logger.warning(f"{country}: No data found in JSON file for {today_str}. Skipping.")
            continue
