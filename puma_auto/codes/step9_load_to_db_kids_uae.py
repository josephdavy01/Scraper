...
def remap_gender_kids(categories, title):
    text = ' '.join([cat.get('name', '').lower() for cat in categories])
    full_text = text + ' ' + title.lower()
    tokens = set(re.findall(r'\b\w+\b', full_text))
    
    has_girl_keywords = any(word in {'girl', 'girls'} for word in tokens)
    has_boy_keywords = any(word in {'boy', 'boys'} for word in tokens)
    
    if has_girl_keywords and has_boy_keywords:
        return 'unisex'
    elif has_girl_keywords:
        return 'female'
    elif has_boy_keywords:
        return 'male'
    else:
        return 'unisex'
...
def create_individual_json(today_str, json_data):
    ...
    mapped_gender = extract_gender(item_categories,name)
    if mapped_gender!='kids':
        return []
    else:gender=remap_gender_kids(item_categories,name)
