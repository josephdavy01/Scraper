...
def extract_material_composition(product_dict):
    ...
    def get_material(key, text):
        match = re.search(fr'{key}:\s*(.*?)(?=\s*[A-Z][a-zA-Z]+:|$)', text, re.IGNORECASE)
        if match:
            return match.group(1).strip()
        return None

    sole = get_material('Outsole', cleaned_text)
    upper = get_material('Upper', cleaned_text)

    return sole, upper, cleaned_text
...
def extract_activity(item):
    for a in item.get('attributes', []) or []:
        if a.get('attribute_code') == 'activity_group' and isinstance(a.get('attribute_options'), list) and a['attribute_options']:
            return (a['attribute_options'][0].get('label') or '').strip().lower()
    return None


def extract_shoe_type(item):
    for a in item.get('attributes', []) or []:
        if a.get('attribute_code') == 'article_type' and isinstance(a.get('attribute_options'), list) and a['attribute_options']:
            return (a['attribute_options'][0].get('label') or '').strip().lower()
    return None

def is_footwear(item):
...
def create_individual_json(today_str, json_data):
    ...
    shoe_type = extract_shoe_type(item)
    ...
    for variant in variants:
        ...
        sole, upper, cleaned = extract_material_composition(vproduct)
        ...
        full_description = remove_html(raw_description) + (cleaned or "")
