...
COLLECTION_PREFIX = 'crawler_sink_zara_'

MON_WED_FRI = ['australia', 'canada', 'india', 'saudi', 'spain']
TUE_THU_SAT = ['turkey', 'uae', 'uk', 'usa']

today_str = datetime.now().strftime('%Y-%m-%d')
# today_str='2025-11-08'
THRESHOLD_PERCENT = 100 # Don't upload if today's count differs by more than the set percentage
...
for country in active_countries:
    ...
    kids_collection_name = f"{COLLECTION_PREFIX}{country}_kids"
    server_collection = server_db[collection_name]
    local_collection = local_db[collection_name]
    kids_local_collection = local_db[kids_collection_name]

    today_count = get_today_count(local_collection, today_str) + get_today_count(kids_local_collection, today_str)
    if today_count == 0:
        continue
    ...
    if server_data_count:
        if server_data_count == today_count:
            ...
            logging.info(f"Reupload enabled: Deleting existing data for {today_str} in {collection_name}...")
            start = datetime.strptime(today_str, '%Y-%m-%d')
            end = start.replace(hour=23, minute=59, second=59)
            server_collection.delete_many({"date_of_scraping": {"$gte": start, "$lte": end}})

    if should_upload(today_count, previous_counts):
        logging.info(f"Uploading {today_count} documents in chunks to {collection_name}...")
        upload_to_melody(local_collection, server_collection, today_str)
        upload_to_melody(kids_local_collection, server_collection, today_str)
