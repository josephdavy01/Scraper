...
def compare_with_previous_data(countries, today_date):
    ...
    def flatten_nested_dict(nested_dict):
        """Flatten nested dictionary structure to simple key:url pairs"""
        flat_dict = {}
        for category, items in nested_dict.items():
            if isinstance(items, dict):
                for name, url in items.items():
                    # Create a unique key combining category and name
                    flat_key = f"{category} > {name}"
                    flat_dict[flat_key] = url
            else:
                # If it's already flat, just add it
                flat_dict[category] = items
        return flat_dict
    ...
    for country in countries:
        ...
        print(f"Comparing {country} ({today_date} vs {previous_date})")
        
        try:
            with open(current_file, 'r') as f:
                current_data_raw = json.load(f)
            with open(previous_file, 'r') as f:
                previous_data_raw = json.load(f)
            
            # Flatten the nested structure for comparison
            current_data = flatten_nested_dict(current_data_raw)
            previous_data = flatten_nested_dict(previous_data_raw)
    ...
    if level == 'category':
        for country in countries:
            try:
                ...
                file_path = os.path.join(country_path, f"{country}_category_urls.json")
                ...
                # Handle nested structure: {"Men": {...}, "Women": {...}, "Kids": {...}}
                    ...
                    if isinstance(items, dict):
                        ...
                        for name, url in items.items():
                            if url not in seen_urls:
                                seen_urls.add(url)
                                cleaned_category[name] = url
                            else:
                                duplicates_log.append({"category": category, "path": name, "url": url})
                        cleaned_data[category] = cleaned_category
                    else:
                        # If it's a flat structure, handle it directly
                        if items not in seen_urls:
                            seen_urls.add(items)
                            cleaned_data[category] = items
                        else:
                            duplicates_log.append({"path": category, "url": items})
        ...
        for country in countries:
            try:
                ...
                file_path = os.path.join(country_path, f"{country}_product_urls.json")
                if not os.path.exists(file_path):
                    continue
                with open(file_path, 'r') as f:
                    data = json.load(f)
...
def compare_product_urls(countries, today_date):
    """Compare today's product urls with previous day's product urls and log the counts and deviation."""
    for country in countries:
        try:
            country_path = os.path.join(country)
            current_file_path = os.path.join(country_path, today_date, 'Item_urls', f"{country}_product_urls.json")

            if not os.path.exists(current_file_path):
                print(f"No current product urls file found for {country} on {today_date}")
                continue

            previous_date = find_previous_date_folder(country_path, today_date)
            if not previous_date:
                print(f"No previous product urls folder found for {country}")
                continue

            previous_file_path = os.path.join(country_path, previous_date, 'Item_urls', f"{country}_product_urls.json")

            if not os.path.exists(previous_file_path):
                print(f"No previous product urls file found for {country} in folder {previous_date}")
                continue
            ...
            print(f"Error comparing product urls for {country}: {e}")


def summarize_product_url_changes(countries, today_date):
    """
    For each country, compare today's product urls with the most recent previous day's product urls
    ...
    for country in countries:
        try:
            ...
            previous_date = find_previous_date_folder(country_path, today_date)
            if not previous_date:
                print(f"No previous product urls folder found for {country}. Skipping summary.")
                continue
...
def check_deviation(log_file_path, total_urls_file_path):
    ...
    if isinstance(log_data, list):
        successful_scrapes = len(log_data)
    elif isinstance(log_data, dict):
        for url, info in log_data.items():
            if info.get('status') == 'done':
                successful_scrapes += 1
    else:
        print(f"Unknown log data format in {log_file_path}")
        return 100.0
