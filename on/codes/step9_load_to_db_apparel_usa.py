...
def is_apparel(json_data):
    ...
    footware_keywords = ['apparels', 'apparel']
    return any(keyword in variant_url for keyword in footware_keywords)

def extract_composition(material_data):
    ...
    if isinstance(material_data, dict):
        ...
        if materials and isinstance(materials, list):
            return '; '.join(str(m) for m in materials if m)
        return None
...
def create_individual_json(today_str, json_data, gender):
    ...
    if not sizes:
        sizes = [{'size_name': 'One Size', 'in_stock': True}]
...
def process_jsons(today_str, country, collection):
    ...
    if not os.path.exists(gender_folder):
        print(f"Data folder not found: {gender_folder}")
        return
    ...
    for gender in genders:
        ...
        for category in categories:
            ...
            files = [f for f in os.listdir(file_folder) if f.endswith('.json')]
            for file in files:
    ...
    print(f"{country} Summary: {total_processed} products processed, {total_skipped} skipped")
