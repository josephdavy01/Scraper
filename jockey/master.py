...
from step1_category_urls import category_urls
from step2_product_urls import product_urls
from step3_variant_product_urls import variant_product_urls as variant_data
...
COUNTRIES = {'india':"https://jockey.in/"}
...
MONGO_CONFIG_APPAREL = {
    ...
    "DB_NAME": "tg_analytics",
    "COLLECTION_PREFIX": "crawler_sink_jockey",
...
async def main():
    ...
    if EXECUTION_CONFIG.get("run_step2_product_urls"):
        ...
        for country in COUNTRIES:
            ...
            if os.path.exists(log_file_path):
                ...
                raise_ticket("Master", "compare_product_urls", f"No comparison log found for {country} on {TODAY_DATE}")

    # ---------------------- STEP 3 ---------------------- #
    if EXECUTION_CONFIG.get("run_step3_variant_data"):
        logging.info("\nStep 3: Fetching all variant data")

        try:
            await variant_data()
        except Exception as e:
            logging.error(f"Step 3 error: {e}")
            raise_ticket("Master", "run_variant_data_scraper", f"Step 3 error: {e}")
    ...
    # ---------------------- STEP 4 ---------------------- #
    if EXECUTION_CONFIG.get("run_step4_daily_count"):
        logging.info("\nStep 4: Daily count")
        try:
            for country in COUNTRIES:
                process_daily_count(country, TODAY_DATE)
        except Exception as e:
            logging.error(f"Step 4 error: {e}")
            raise_ticket("Master", "process_country_data", f"Step 4 error: {e}")
    # ---------------------- STEP 5 ---------------------- #
    if EXECUTION_CONFIG.get("run_step5_get_unique_product_urls"):
        logging.info("\nStep 5: Getting unique product URLs")
        try:
            for country in COUNTRIES:
                get_unique_urls(country, TODAY_DATE)
        except Exception as e:
            logging.error(f"Step 5 error: {e}")
            raise_ticket("Master", "get_unique_urls", f"Step 5 error: {e}")

    # ---------------------- STEP 6 ---------------------- #
    if EXECUTION_CONFIG.get("run_step6_product_data"):
        logging.info("\nStep 6: Fetching detailed product data")
        try:
            await product_data_main()
