...
server_DB_NAME = 'footwear_analytics'
COLLECTION_PREFIX = 'crawler_sink_asics_'

MON_WED_FRI = ['uk', 'usa']
TUE_THU_SAT = ['india', 'uae']

today_str = datetime.now().strftime('%Y-%m-%d')
# today_str = '2025-12-09'
# today_str = '2025-11-29'

THRESHOLD_PERCENT = 10.0  # Don't upload if today's count differs by more than the set percentage
...
server_db = server_client[server_DB_NAME]
local_client = pymongo.MongoClient(LOCAL_MONGO_URI)
local_db = local_client[local_DB_NAME]

# ---------------------------------- UTILITY FUNCTIONS ----------------------------------

def upload_to_melody(source_collection, target_collection, today_str, force_upload=False):
    BATCH_SIZE = 1000
    total_docs = 0
...
for country in active_countries:
    local_collection_name = f"{COLLECTION_PREFIX}{country}_footwear"
    ...
    if server_data_count:
        if server_data_count == today_count:
            ...
            logging.info(f"Reupload enabled: Deleting existing data for {today_str} in {server_collection_name}...")
            start = datetime.strptime(today_str, '%Y-%m-%d')
            end = start.replace(hour=23, minute=59, second=59)
            server_collection.delete_many({"date_of_scraping": {"$gte": start, "$lte": end}})

    if should_upload(today_count, previous_counts):
        logging.info(f"Uploading {today_count} documents in chunks to {server_collection_name}...")
