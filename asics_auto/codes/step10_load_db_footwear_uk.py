...
def is_footwear(j):
    keywords = ("shoe", "shoes", "sneaker", "sneakers",'footwear', 'footwears')
    text = get_product_text(j)
    return any(k in text for k in keywords)


def extract_materials(accordion_sections):
    if not accordion_sections:
        return None, None
    
    description_section = accordion_sections.get('Description', {})
    tech_section = accordion_sections.get('Tech & Features', {})
    details_section = accordion_sections.get('Details', {})
    
    description = description_section.get('content', '')
    details = tech_section.get('content', '')
    details_data = details_section.get('content','')
    full_discription= description + "\n" + str(details or '')
    
    weight_match = re.search(r'Weight\s+([\d\.]+\s*\w+\/[\d\.]+\s*\w+)', details_data)
    weight = weight_match.group(1) if weight_match else None

    heel_drop_match = re.search(r'Heel Drop\s+([\d\.]+\s*\w+)', details_data)
    heel_drop = heel_drop_match.group(1) if heel_drop_match else None

    return weight, heel_drop,full_discription
...
def map_base_sku(all_sizes_data, target_size):
    for size_obj in all_sizes_data:
        if size_obj.get("data-sizevalue") == target_size and size_obj.get("link_data-select") == "js-mens-size":
            base_sku = size_obj.get("link_data-sku", "").strip()
            return base_sku
    return None
...
def create_individual_json(today_str, json_data, gender):
    ...
    weight, heel_drop,full_discription = extract_materials(json_data.get('accordion_sections'))

    price_info = json_data.get('price_info', {})
    
    # Safely convert price strings to float, removing currency symbols
    original_price_str = price_info.get('meta_price', '0').replace('£', '').strip()
    current_price_str = price_info.get('current_price', '0').replace('£', '').strip()
    
    try:
        launch_price = float(original_price_str) if original_price_str else 0.0
    ...
    # Use a clear variable name for the product ID
