...
def process_country_data(country, fetch_date):
    ...
    input_file = os.path.join(folder, 'products_data_' + country + '.json')
    output_file = os.path.join(folder, 'products_data_deduped_' + country + '.json')
    log_file = os.path.join(folder, f'duplicates_log_{fetch_date}.json')
...
def remove_duplicate_sku(countries, fetch_date, re_run=False):
    #Check if the data file exists  
    for country in countries.keys():
        folder = os.path.join(country, fetch_date, 'Data')
        input_file = os.path.join(folder, f"products_data_deduped_{country}.json")
...
if __name__ == "__main__":
    ...
    remove_duplicate_sku(COUNTRIES,TODAY_DATE)