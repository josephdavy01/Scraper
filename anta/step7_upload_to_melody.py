...
def read_json_data(country, today_str, file_type):
    file_path = os.path.join(country, today_str, 'Final_json', f'{country}_{file_type}_data.json')
    ...
    try:
        ...
        raise_ticket( "read_json_data", f"Error reading JSON file {file_path}: {str(e)}", country)
        return []
...
def upload_to_data_melody(countries, today_str, config):
    ...
    try:
        server_client = pymongo.MongoClient(config['LOCAL_MONGO_URI'])
        server_db = server_client[config['DB_NAME']]
        
        logger.info("Connected to server database successfully.")
    except Exception as e:
        logger.error(f"Error connecting to database: {e}")
        raise_ticket("upload_to_data_melody", f"Database connection failed: {str(e)}")
        return
...
def main():
    ...
    # MongoDB configuration for footwear
    FOOTWEAR_CONFIG = {
        "LOCAL_MONGO_URI": os.getenv("SERVER_MONGO_URI"),
        "DB_NAME": "footwear_analytics",
        "COLLECTION_PREFIX": "crawler_sink_anta_",
        "THRESHOLD_PERCENT": 10.0,
        "FORCE_UPLOAD": False,
        "DRY_RUN": False,
    }
    
    # MongoDB configuration for apparel
    APPAREL_CONFIG = {
        "LOCAL_MONGO_URI": os.getenv("SERVER_MONGO_URI"),
    ...
    # Upload footwear data
    logger.info("=" * 60)
    logger.info("Starting FOOTWEAR data upload...")
    logger.info("=" * 60)
    upload_to_data_melody(COUNTRIES, TODAY_DATE, FOOTWEAR_CONFIG)
    
    # Upload apparel data
    logger.info("=" * 60)
    logger.info("Starting APPAREL data upload...")
    logger.info("=" * 60)
    upload_to_data_melody(COUNTRIES, TODAY_DATE, APPAREL_CONFIG)
