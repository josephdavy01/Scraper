...
def create_individual_json(json_data,today_str):
    ...
    try:
        ...
        name = unicodedata.normalize('NFKD', json_data['title']).encode('ascii', 'ignore').decode('ascii').lower().strip()
        gender = remap_gender(json_data)
        # normalizing unicode characters using unicodedata.normalize  
        descriptions_parts = json_data.get("description", "")       
        descriptions_regx = re.sub(r'<.*?>', '', descriptions_parts)
        if descriptions_regx:
            descriptions = unicodedata.normalize('NFKD', descriptions_regx).encode('ascii', 'ignore').decode('ascii').strip().replace("\n", "").replace("—", "").replace("'", "'")
        else:
            descriptions = ""

        composition_text = json_data.get("composition")
        if composition_text:
            composition_text = unicodedata.normalize('NFKD', composition_text).encode('ascii', 'ignore').decode('ascii')
            match = re.search(r'(\d+%.*?)(?:,\s*Product code:|$)', composition_text) or re.search(r'(Material:.*?)(?:,\s*Product code:|$)', composition_text) or re.search(r'(Fabric:.*?)(?:,\s*Product code:|$)', composition_text)
            composition = match.group(1).strip() if match else composition_text
        else:
            composition = ""
...
key = ['Shoes','Socks','Slides','Sneakers','Sandals','Boots','Bags','Belts','Headwear','Bags','Balls']
    ...
    for category in categories:
        ...
        for file in files:
            ...
            try:
                ...
                if any(k.lower() in product_type.lower() for k in key):
                    continue
