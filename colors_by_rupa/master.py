...
from step3_daily_count import process_country_data as process_daily_count
from step4_get_unique_product_urls import process_country_data as get_unique_urls
from step5_product_data import get_product_data_main as product_data_main
from step6_urls_json_comparison import compare_urls_json_comparisons
from step7_update_cid import run_cid_mapping_tts as update_cid
from step8_process_json import save_country_data_to_json as process_json
...
EXECUTION_CONFIG = {
    ...
    "run_step6_urls_json_comparison": True,
    "run_step7_update_cid": True,
...
async def main():
    ...
    # ---------------------- STEP 3 ---------------------- #
    if EXECUTION_CONFIG.get("run_step3_get_daily_count"):
        logging.info("\nStep 3: Daily count")
        try:
            for country in COUNTRIES:
    ...
    # ---------------------- STEP 6 ---------------------- #
    if EXECUTION_CONFIG.get("run_step6_urls_json_comparison"):
        logging.info("\nStep 6: URLs JSON comparison")
        try:
            compare_urls_json_comparisons()
        except Exception as e:
            logging.error(f"Step 6 error: {e}")
            raise_ticket("Master", "compare_urls_json_comparisons", f"Step 6 error: {e}")

    # ---------------------- STEP 8 ---------------------- #
    if EXECUTION_CONFIG.get("run_step8_process_json"):
        logging.info("\nStep 8: Processing JSON data")
        try:
            process_json(COUNTRIES, TODAY_DATE, re_run=False)
        except Exception as e:
