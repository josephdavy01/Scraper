...
def parse_proxy(proxy):
    # Accepts ip:port:username:password and ip:port (no authentication)
        ...
        try:
            async with async_playwright() as p:
                ...
                if "Access Denied" in content or "verify you are human" in content:
                    logging.warning(f"Access denied: {url} [{proxy_str or 'direct'}]")
                    if not use_proxy:
                        ...
                        attempt_number += 1
                        continue
                # --- scraping logic below: adjust CSS selectors as needed ---
                try:
