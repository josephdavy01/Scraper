...
def process_country_data(country, today_str):
    ...
    try:
        ...
        for category, plist in pids_dict_1.items():
            if len(plist) > 0:  # Filter out categories with zero count
                p1_total += len(plist)
                p_count_list_1.append({'Category': category, 'Count': len(plist)})

        p_count_list_1.append({'Gender': "all", 'Category': "all", 'Count': p1_total})

        for category, plist in pids_dict_2.items():
            if len(plist) > 0:  # Filter out categories with zero count
                p2_total += len(plist)
                p_count_list_2.append({'Category': category, 'Count': len(plist)})
