...
def get_age_range(size):
    ...
    if "first" in size:
        ...
        age_range.append('0m')
        range_val = int(size.split('to')[1].split('m')[0].strip())
        age_range.append(str(range_val) + 'm')
    elif 'm' in size or 'y' in size:
        size = size.split(' ')[-1]
...
def create_individual_json(today_str, product, gender):
    ...
    for size in product['options']['options']:
        ...
        # Handle both age-based and shoe-size-based items
        ...
        if product['priceData']['wasPrice'] == None:
            ...
            dis_percentage = int(((((oldmax - newmax)/oldmax) + ((oldmin - newmin)/oldmin))/2) * 100) if oldmax > 0 and oldmin > 0 else 0
            oldprice = float(round(price/(100-dis_percentage)*100)) if (100-dis_percentage) > 0 else price
...
if __name__ == "__main__":
    ...
    for country in countries:
        collection = db[f'crawler_sink_next_{country.lower()}_kids_footwear']
