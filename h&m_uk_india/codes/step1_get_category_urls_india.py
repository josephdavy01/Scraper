    ...
    for gender_dict in json_data:
        ...
        if gender in ['ladies' ,'women', 'men']:
            ...
            for sub_gender in gender_dict['children']:
                ...
                for age_group in sub_gender['children']:
                    age_group_name = age_group['nodeName'].lower().replace(' ', '-').replace('&', 'and').strip()
                    for main_category in age_group['children']:
                        main_category_name = main_category['nodeName'].lower().replace(' ', '-').strip()
                        if main_category_name in ['clothing', 'sportswear']:  # Adjusted to match actual names; use 'sportswear' instead of 'sport'
                            for category in main_category['children']:
                                category_name = category['nodeName'].lower().replace(' ', '-').strip()
                                if '-all' in category_name:
                                    url = 'https://www2.hm.com' + category['href']
                                    fname = f'{sub_gender_name}_{age_group_name}_{main_category_name}_{category_name}'
                                    if 'toys' not in fname and 'accessories' not in fname:
