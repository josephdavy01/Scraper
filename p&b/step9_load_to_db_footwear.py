...
def create_individual_json(c_code, today_str, json_data):
    ...
    for index, color in enumerate(colors):
        ...
        upper, lining, sole, insole= extract_composition_details(color['compositionDetail']) if 'compositionDetail' in color else ('', '', '', '')
            ...
            for file in files:
                ...
                try:
                    ...
                    if product_type != 'footwear':
                        print(f" {product_type} not footwear so ,skipping!!!")
                        continue

                    bundle_summaries = data.get('bundleProductSummaries', [])
              
                    for summary in bundle_summaries:
                        family_name = summary.get('familyName')
                        if family_name:
                            fn = family_name.strip().lower()
                        
                            skip_keywords = [
                                'accessories', 'bags', 'backpack', 'handbag', 'wallet', 'belt', 'scarf', 'scarves', 'foulard',
                                'hat', 'sunglasses', 'jewelry', 'imitation jewelry','sock','interiors', 'plush', 'cradle'
                            ]

                            if any(keyword in fn for keyword in skip_keywords):
                                print(f" {fn} contains non-clothing keywords, skipping!!!")
                                continue
...
if __name__ == "__main__":
   ...
   # Database details
    ...
    for country, code in countries.items():
        collection = db[f'crawler_sink_pull&bear_{country.lower()}_footwear']
