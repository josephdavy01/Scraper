...
def check_data_format(connection_string, database_name, collection_name, geography, date):
    ...
    if any(age_range not in ['1m', '2m', '3m', '4m', '5m', '6m', '7m', '8m', '9m', '10m', '11m', '12m', '13m', '14m', '15m', '16m', '17m', '18m', '19m', '20m', '21m', '22m', '23m', '24m', '2y', '3y', '4y', '5y', '6y', '7y', '8y', '9y', '10y', '11y', '12y', '13y', '14y', '15y', '16y', '17y', '18y'] for age_range in age_ranges):
        format_check = 1
    ...
    if format_check == 0:
        ...
        logging.error(f"Data format check failed for {geography} on {date}. Please review the errors above.")
...
# 
    ...
    if day in ['Monday', 'Wednesday', 'Friday']:
        collections = {
                'Australia' : 'crawler_sink_pull&bear_australia',
                'Saudi' : 'crawler_sink_pull&bear_saudi',
                'Spain' : 'crawler_sink_pull&bear_spain',
                'Turkey' : 'crawler_sink_pull&bear_turkey'
            }
    elif day in ['Tuesday', 'Thursday', 'Saturday']:
        collections = {
                'UAE' : 'crawler_sink_pull&bear_uae',
                'UK' : 'crawler_sink_pull&bear_uk',
                'USA' : 'crawler_sink_pull&bear_usa'
            }
    else:
        collections = { }
        
    for geography, collection_name in collections.items():
