...
def check_data_format(connection_string, database_name, collection_name, geography, date):
    ...
    if any(gender not in ['male', 'female', 'unisex','kids'] for gender in genders):
        format_check = 1
        logging.error(f"Data format error in {geography} for date {date}: 'gender' contains invalid values.{genders}")
...
if __name__ == "__main__":
    ...
    if day in ['Tuesday', 'Thursday', 'Saturday']:
        collections = {
            'UK' : ['crawler_sink_riverisland_uk', 'crawler_sink_riverisland_uk_kids'],
            'USA' : ['crawler_sink_riverisland_usa', 'crawler_sink_riverisland_usa_kids']
        }
    else:
