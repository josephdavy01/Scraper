...
class ScraperManager:
    ...
    import portalocker  # Windows-compatible file locking

    def _save_results(self, gender, category, product_urls):
        """Save results with proper file locking"""
        # Use file-based locking instead of multiprocessing Lock
        lock_file = self.results_file.with_suffix('.lock')
        
        with open(lock_file, 'w') as lock:
            portalocker.lock(lock, portalocker.LOCK_EX)  # Exclusive lock
