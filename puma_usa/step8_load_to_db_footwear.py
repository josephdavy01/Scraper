...
def upload_to_mongodb_in_chunks(data, country, today_date, db_config):
    if not data:
        print(f"[{country}] No footwear data to upload.")
        return

    force_upload = db_config.get('FORCE_UPLOAD', False)
    client = MongoClient(db_config['LOCAL_URI'])
    db = client["footwear_analytics"] # Specific DB for footwear
    ...
    if existing_count > 0:
        if force_upload:
            print(f"[{country}] FORCE_UPLOAD is True. Deleting {existing_count} existing footwear documents for {today_date}.")
            collection.delete_many(date_filter)
        else:
            print(f"[{country}] Footwear data for {today_date} is already uploaded ({existing_count} docs). Skipping.")
            return
    ...
    for batch in chunked(data, CHUNK_SIZE):
        ...
        try:
            ...
            print(f"[{country}] Error inserting footwear batch: {e}")

    print(f"[{country}] Inserted {total_inserted} new footwear documents into collection 'crawler_sink_puma_{country.lower()}'.")

def load_to_db_footwear(TODAY_DATE, COUNTRIES, db_config):
    for country in COUNTRIES:
        output_dir = Path(country) / TODAY_DATE / "Data"
        combined_file = output_dir / f"footwear_products_data_deduped_{country}.json"

        if not combined_file.exists():
            print(f"[{country}] Footwear file not found: {combined_file}")
            continue
        ...
        try:
            ...
            print(f"[{country}] Failed to load footwear JSON: {combined_file}")
            continue
