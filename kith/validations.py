...
def find_duplicate_urls(data):
    ...
    for category, sub_cats in data.items():
        for sub_cat, items in sub_cats.items():
            if isinstance(items, dict):
                for name, url in items.items():
                    url_map[url].append(f"{category} > {sub_cat} > {name}")
...
def compare_json_data(current_data, previous_data):
    ...
    # Create URL to key mapping (URL -> List of full paths)
    previous_url_map = defaultdict(list)
    current_url_map = defaultdict(list)
    
    for cat, sub_cats in previous_data.items():
        for sub_cat, items in sub_cats.items():
            if isinstance(items, dict):
                for name, url in items.items():
                    previous_url_map[url].append(f"{cat} > {sub_cat} > {name}")
    
    for cat, sub_cats in current_data.items():
        for sub_cat, items in sub_cats.items():
            if isinstance(items, dict):
                for name, url in items.items():
                    current_url_map[url].append(f"{cat} > {sub_cat} > {name}")

    # Detect renamed/moved items (URL is the same, path is different)
    ...
    # These are URLs that exist in one file but not the other at all.
    ...
    for url in added_urls:
        for path in current_url_map[url]:
            category = path.split(' > ')[0]
            if 'added' not in differences['changes'][category]:
                differences['changes'][category]['added'] = {}
            differences['changes'][category]['added'][path] = url

    for url in removed_urls:
        for path in previous_url_map[url]:
            category = path.split(' > ')[0]
            if 'removed' not in differences['changes'][category]:
                differences['changes'][category]['removed'] = {}
            differences['changes'][category]['removed'][path] = url

    # --- Identify MODIFIED URLs ---
    # This is when the path is the same, but the URL is different.
    
    # Create path -> URL maps
    previous_path_map = {path: url for url, paths in previous_url_map.items() for path in paths}
    current_path_map = {path: url for url, paths in current_url_map.items() for path in paths}

    common_paths = set(previous_path_map.keys()) & set(current_path_map.keys())
    for path in common_paths:
        if previous_path_map[path] != current_path_map[path]:
            category = path.split(' > ')[0]
            if 'modified' not in differences['changes'][category]:
                differences['changes'][category]['modified'] = {}
            differences['changes'][category]['modified'][path] = {
                'old': previous_path_map[path],
                'new': current_path_map[path]
            }
            
    # Clean up empty categories from changes
    differences['changes'] = {k: v for k, v in differences['changes'].items() if v}
...
def compare_with_previous_data(countries, today_date):
    ...
    for country in countries:
        ...
        current_file = os.path.join(country_path, today_date, "Category", f"{country}_category_links.json")
        ...
        previous_file = os.path.join(country_path, previous_date, "Category", f"{country}_category_links.json")
        
        print(f"\nComparing {country} ({today_date} vs {previous_date})")
        
        try:
            with open(current_file, 'r') as f:
                current_data = json.load(f)[0]
            with open(previous_file, 'r') as f:
                previous_data = json.load(f)[0]
    ...
    if level == 'category':
        for country in countries:
            try:
                ...
                file_path = os.path.join(country_path, f"{country}_category_links.json")
                        ...
                        if isinstance(items, dict):
                            ...
                            for name, url in items.items():
                                if url not in seen_urls:
                                    ...
                                    duplicates_log.append({"path": f"{category} > {sub_cat} > {name}", "url": url})
                            cleaned_sub_cats[sub_cat] = cleaned_items
                    cleaned_data[category] = cleaned_sub_cats
...
def check_deviation(log_file_path, total_urls_file_path):
    ...
    for url, info in log_data.items():
    ...
    if not os.path.exists(total_urls_file_path):
        ...
        # If we can't determine total, we can't calculate deviation.
        # Returning 0 might be misleading. Let's return -1 to indicate an error.
        return -1.0 
    ...
    # Count total URLs
    total_urls = count_urls_in_json(total_urls_data)

    if total_urls == 0:
        return 0.0  # No URLs to process, so 0 deviation.
