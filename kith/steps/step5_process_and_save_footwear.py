...
def is_footwear(product_type):
    """Checks if a product is footwear based on its type."""
    pt = product_type.lower() if product_type else ''
    return any(keyword in pt for keyword in ['low top sneakers', 'high top sneakers', 'mid top sneakers', 'sneakers', 'boots', 'sandals', 'slides', 'loafers', 'mules & clogs', 'flats', 'slippers'])

def extract_info(content):
    """Extracts weight and heel-to-toe drop from product content."""
    weight = None
    heel_to_toe_drop = None
    if content:
        weight_match = re.search(r'(\d+)\s?g(?:rams?)?', content.lower())
        if weight_match:
            weight = int(weight_match.group(1))

        heel_drop_match = re.search(r'heel[- ]to[- ]toe drop\s*:?\s*(\d+)', content.lower())
        if heel_drop_match:
            heel_to_toe_drop = int(heel_drop_match.group(1))
    return weight, heel_to_toe_drop

def get_image_style(image_list, country):
    """Gets image styles, applying country-specific logic."""
    config = COUNTRY_CONFIG.get(country, {})
    image_style_index = config.get('image_style_index', 3)  # Default to USA's 3
    ...
    for index, image in enumerate(image_list):
        if isinstance(image, str):
            ...
            image_style = 's0'
            if 'PAIR' in image.upper() or index == image_style_index:
                image_style = 'n_f_f_c'
...
def create_individual_json(date_dir, json_data, file_name, pdict, cdict, country):
    ...
    if not is_footwear(product_json.get("type", "")):
        logging.info(f"Skipping file {file_name} - category '{product_json.get('type', '')}' not footwear")
        return []
    ...
    content_clean = remove_html(content)
    weight, heel_to_toe_drop = extract_info(content_clean)
        ...
        for file in files:
            if file.endswith('.json'):
                ...
                try:
                    ...
                    shoe_list = create_individual_json(date_dir, json_data, file, pdict, cdict, country)
                    if shoe_list:
                        all_country_products.extend(shoe_list)
...
def process_footwear(countries, today_date, re_run=False):
    """Main function to process footwear data for specified countries."""
    ...
    for country in countries:
        logging.info(f"Processing {country}/{today_date}")
        output_dir = os.path.join(country, today_date, 'Data')
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir, f'{country}_data_footwear.json')

        if not re_run and os.path.exists(output_path):
            # If file exists and is not empty, skip
            if os.path.getsize(output_path) > 0:
                logging.info(f"File {output_path} already exists and is not empty. Skipping.")
                continue
            else:
                logging.info(f"File {output_path} exists but is empty. Reprocessing.")
        ...
        if all_products:
            ...
            logging.info(f"Successfully saved {len(all_products)} products to {output_path}")
        else:
            logging.warning(f"No footwear products found for {country} on {today_date}")
