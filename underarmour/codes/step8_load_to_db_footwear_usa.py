...
def create_individual_json(today_str, json_data, file_name):
    ...
    offset = str(json_data.get("offset",None)).strip()
    if offset in ["None", "", "null", None]:
        offset = None

    images_list = json_data.get('image')
 
    activity = str(json_data.get("category", "")).strip().lower()
    activity = activity.split(">")[-1].strip()

    if "shoes" in activity or "sandals & slides" in activity or "halo shoes & clothing" in activity or "shoes under $100" in activity:
        activity = None
    elif "trail & hiking" in activity or "hiking & tactical" in activity or "military & tactical" in activity or "hiking & hunting" in activity or "military & tactical boots" in activity:
        activity = "hiking"
    elif "running & training" in activity:
        activity = "running"
    elif activity == "sportswear":
        activity = "activewear"
            ...
            for file in files:
                ...
                try:
                    ...
                    if shoe_list:
                        ...
                        if category_value and "Shoes" in category_value:
                            if category_value != "Curry Brand Shoes and Gear": 
                                ...
                                logging.info(f"Skipped category '{category_value}' from {file_path}")
