...
def create_individual_json(today_str, json_data, file_name):
    ...
    if not current_price:
        current_price = json_data.get("sale_price")
    launch_price = None  
    launch_price = json_data.get('original_price')
    if not launch_price:
        launch_price = json_data.get('Price')

    weight = json_data.get("weight",None)
    offset = str(json_data.get("offset",'')).strip()
    if weight == "":
        weight = None
    if offset == "":
        offset = None 
    ...
    activity = str(json_data.get("category", "")).lower().strip()
    activity = activity.split(">")[-1].strip()
    if activity in ["shoes", "sandals & slides"]:
        activity = None
    elif activity in ["trail & hiking", "hiking & tactical"]:
        activity = "hiking"
    elif activity in ["sportswear"]:
        activity = "activewear"
